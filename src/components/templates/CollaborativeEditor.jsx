...
export default function CollaborativeEditor({ template, currentUser, onSave, onClose }) {
  ...
  const { data: comments = [] } = useQuery({
    ...
    queryFn: async () => {
      const allComments = await apiClient.entities.TemplateComment.list('-created_date');
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="edit" className="space-y-4">
          <PermissionGuard 
            ...
            requiredPermission={canEdit ? "edit" : "view"}
            fallback={
              <Alert>
                <Eye className="h-4 w-4" />
                <AlertDescription>
                  You have {userRole} access to this template. {!canEdit && "Request edit access from the owner."}
                </AlertDescription>
              </Alert>
            }
          />
