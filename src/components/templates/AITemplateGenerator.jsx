...
export default function AITemplateGenerator({
  ...
  const fetchAvailableModels = async () => {
    ...
    try {
      const { data } = await apiClient.functions.invoke('ollamaProxy', { endpoint: selectedEndpoint, action: 'list-models' });
  ...
  const generateWithapiClient = async (systemPrompt, userPrompt, schema) => {
...
- Placeholders: Array of objects describing each placeholder found in 'Content'. Each object should have 'key' (the placeholder name without braces), 'label' (a human-readable label), 'description' (what the placeholder represents), and 'default' (an optional default value or example text if appropriate).`;
        ...
        if (useOllama) {
          ...
          result = await generateWithapiClient(systemPrompt, userPrompt, schema);
        }
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          {!ollamaConfigured && useOllama && (
            <Alert className="mb-4">
              ...
              <AlertDescription>
                No Ollama endpoints configured. Please add endpoints in settings or disable "Use Ollama" to proceed with apiClient AI.
              </AlertDescription>
            </Alert>
          )}
          ...
          >
            {isGenerating ? (
              <>
                ...
                Generating with {useOllama ? `Ollama (${selectedModel})` : 'apiClient AI'}...
              </>
            ) : (
              <>
